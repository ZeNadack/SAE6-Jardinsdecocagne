...
@section('content')
    <div class="container">
        <h1>Créer une nouvelle tournée</h1>
        ...
        <form action="{{ route('tournees.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="date" name="jour_preparation" id="jour_preparation" class="form-control" required>
            </div>
            <div class="form-group">
                <label for="jour_livraison">Jour de livraison</label>
                <input type="date" name="jour_livraison" id="jour_livraison" class="form-control" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group espace">
                ...
                <select name="point_depots[]" id="point_depots" class="form-control" multiple required>
                    @foreach ($pointDepots as $pointDepot)
                        <option value="{{ $pointDepot->id }}" data-latitude="{{ $pointDepot->latitude }}"
                            data-longitude="{{ $pointDepot->longitude }}">
                            {{ $pointDepot->nom }}
                        </option>
                    @endforeach
                </select>
                <small class="form-text text-muted">Maintenez la touche Ctrl (ou Cmd) pour sélectionner plusieurs points de
                    dépôt.</small>
            </div>

            <button type="submit" class="btn btn-primary espace">Créer</button>
        </form>
        ...
        <script>
            var map = L.map('map').setView([48.8566, 2.3522], 13); // Centré sur Paris par défaut
